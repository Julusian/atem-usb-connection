...
import { CommandParser } from 'atem-connection/dist/lib/atemCommandParser'
import exitHook = require('exit-hook')
import { VersionCommand, ISerializableCommand, IDeserializedCommand } from 'atem-connection/dist/commands'
import { DEFAULT_PORT } from 'atem-connection/dist/atem'
...
export class AtemSocketChild {
    ...
    sendCommands(commands2: { payload: number[]; rawName: any; trackingId: number }[]) {
        // throw new Error("Method not implemented.")
    }
...
export class AtemUSBSocket extends EventEmitter {
	...
	private _socketProcess: AtemSocketChild | undefined
	...
	private async _createSocketProcess () {
        const socketProcess = new AtemSocketChild()
		// const socketProcess = await threadedClass<AtemSocketChild, typeof AtemSocketChild>('atem-connection/dist/atemSocketChild', AtemSocketChild, [
		// 	{
	...
	private _parseCommands (buffer: Buffer) {
		...
		while (buffer.length >= 8) {
            ...
            // console.log(buffer.length, name, length)
			if (length < 8) {
